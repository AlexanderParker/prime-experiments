...
class HyperParams:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "duplicate_mutate_rate": self.duplicate_mutate_rate,
        }
...
def create_random_hyperparams() -> HyperParams:
    ...
    return HyperParams(
        ...
        prune_rate=random.uniform(0.0, 0.5),
        duplicate_mutate_rate=random.uniform(0.0, 0.5),
    )
...
def mutate_hyperparams(params: HyperParams, mutation_rate: float = 0.3) -> HyperParams:
    ...
    if random.random() < mutation_rate:
        ...
        if choice == 0:
            ...
            new_params.prune_rate = random.uniform(0.0, 0.5)
        elif choice == 5:
            new_params.duplicate_mutate_rate = random.uniform(0.0, 0.5)
        else:
            keep_pct = random.uniform(0.1, 1.0)
            crossover_pct = random.uniform(0.0, 1.0 - keep_pct)
            random_pct = 1.0 - keep_pct - crossover_pct
...
def evaluate_hyperparams(
    ...
    results = evolve.genetic_algorithm(
        ...
        duplicate_mutate_rate=params.duplicate_mutate_rate,
        verbose=False,
        seed_ast=seed_ast,
    )
                        ...
                        for i, (async_result, index, params) in enumerate(pending_results):
                            if async_result.ready():
                                try:
                                    ...
                                    print(
                                        f"  match_weight_factor={params.match_weight_factor:.3f}, mutation_rate={params.mutation_rate:.3f}, prune_rate={params.prune_rate:.3f}, duplicate_mutate_rate={params.duplicate_mutate_rate:.3f}"
                                    )
...
if __name__ == "__main__":
    ...
    print(f"duplicate_mutate_rate: {best_params.duplicate_mutate_rate:.3f}")
