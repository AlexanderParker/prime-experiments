...
## 7. ROUND 27 - THE CASE-SPLIT CERTIFICATES IN THE KERNEL

Status of this section: **KERNEL-CHECKED**, `proofs/CaseSplit.lean` and
`proofs/CaseCert23*.lean`; the transcription is generated by
`research/gen_case_lean.py` from `research/data/r27/cert_19_23.json`, which is
itself produced and re-verified by `research/lp_cert_lean.py` (gate
`python research/lp_cert_lean.py GATE` -> ALL ASSERTIONS PASSED).

### 7.1 What is new here

Sections 1-6 kernel-check the UNRESTRICTED level-2 covering dual
(`CoveringCert.lean`, `CoveringCert2.lean`: rungs 11->13, 13->17, 17->19).
That vehicle is refuted at 19->23 and beyond - a fact with exhibited exact
primal witnesses (LP thread, round 25). The RESTRICTED species
(`restricted-covering-certificates.md`) escapes those refutations by holding
the phases of the smallest gears and certifying every case separately. This
section is the first time a member of that species has been checked by a
kernel rather than by the script that produced it.

    theorem CaseCert23.D_19_23_case (n : ℕ) : Machine23.g23 n ≤ 25 + 23

with `CaseCert23.F_le : Machine23.g23 n ≤ 48` and
`CaseCert23.no_run : 1 ≤ p → ∃ i < 48, Machine23.Exposed23 (p + i)`.
No census hypothesis, no period, no `native_decide`; the inputs are the seven
primes 5..23 and 574 small integers per case.

### 7.2 The four soundness lemmas, and what each cost

Round 26's sizing (formalist.md R26.8) named four lemmas as the obstacle.
Measured against the artefacts, three of them collapsed and one turned out to
be the whole content:

1. `pos` RESTRICTED - trivial in the end. `pos` is a LITERAL LIST in the Lean
   file, and the only fact needed is `gb5 w (pos t) = false` for every `t`,
   one `decide` per case (`pfree`). The held gear enters nowhere else.
2. `dom(q)` RESTRICTED - VACUOUS at the case-split instance. Domains shrink
   only when OPEN positions are prescribed (the windowed instance); a case
   split prescribes none, so every gear ranges over all its phases.
3. CUT VALIDITY - VACUOUS at every rung on disk. `rows_all_base_cut` is
   asserted for all 75 cases of the three ladder rungs: the separation loop
   added no cut at these widths. The coverage row is `sum_q [q blocks i] >= 1`,
   which is exactly the hypothesis "the window is fully blocked".
4. CASE EXHAUSTIVENESS - one `omega`. `p % 5 = 0 ∨ ... ∨ p % 5 = 4`, then five
   `nocase` applications. The step round 26 called "the one with no analogue
   in the existing files" is the cheapest of the four.

WHAT THE WORK ACTUALLY WAS, and it was not on the list: the RECURSION ROW.
`frow` carries a coefficient `n_ab` per gear pair, defined in the LP thread's
code as a MAX-COVER over the phases of the gears below `a` - an object with no
Lean analogue and, taken literally, 8.2 million evaluations to certify. The
identity that makes it cheap:

    LOWEST-BLOCKER LEMMA.  If position x is blocked by at least one free
    gear, then  1 + #{(a,b) : a < b, both block x, no gear below a blocks x}
                =  #{a : a blocks x}
    because only the LOWEST blocker can be the `a` of such a pair, and it
    pairs with each of the other blockers exactly once.

Summed over `pos` this gives `sum_a |A_a| >= |pos| + sum_{a<b} n_ab` for ANY
`n_ab` that is at most the number of positions where `a` is lowest and `b`
also blocks - which is what the vehicle's `n_ab` is. In Lean it is one
Boolean decision over 2^m assignments (`CaseSplit.lowest6`, `lowest7`), and it
carries NO AXIOMS AT ALL.

### 7.3 Two measured facts about the certificates themselves

* **THE CUTS ARE NEVER USED.** All 75 certified cases of rungs 19->23, 23->29
  and 29->31 have every coverage row equal to the base cut (iterations 0-2,
  and the separated rows that do exist are only in the WINDOWED instances).
  The certifying power at these widths is entirely in the recursion row plus
  the pairwise consistency links.
* **THE RECURSION ROW IS 96% A KOUNIAS ROW AT THE SMALLEST FREE GEAR.** At
  29->31, `n_ab = 0` for 52,173 of the 54,145 gear-index-1 columns: one gear
  below suffices to cover the whole two-gear overlap. Only the pairs whose
  lower gear is the SMALLEST free gear (which has nothing below it) carry a
  systematically nonzero coefficient. Encoding the exceptions as a list cut
  the kernel check from 9m01 to 4m10 per case (solo) at 29->31, and it is the
  reason a 35-case rung is affordable at all.

### 7.4 Cost, for the next lane that wants a rung

    rung     cases  free gears  |pos|  numerals/case  kernel time/case
    19->23      5        6       28-29        574          ~55 s
    29->31     35        7       30-33        978          4m10 solo

(END TO END: the 35-case rung built in 47 MINUTES WALL at two concurrent
workers - 1.34 min/case throughput.)

(numerals actually in the Lean file: `pos` + `y` + `nu` + the exception list;
the |pos| x 22 cut rows are NOT stored, because they are all the base cut.)

The certificate DATA is small (my round-26 measurement stands: ~5 KB per case,
denominators <= 5 bits). What costs is evaluating the BLOCK MAXIMA - one
`aP` value per phase pair per gear pair, ~3,400 columns at 19->23 and ~7,200
at 29->31 - inside `Finset.range` sums. That is the scaling wall of this
vehicle in the kernel, and it grows like the number of columns, i.e. like the
sum of products of pairs of free gears: about x2 per rung, against the case
count's primorial. **In the kernel the case count, not the column count, is
what will bite** - at the measured 1.34 min/case throughput (two workers),
k = 3 (385 cases) is ~8.6 h and k = 4 (5,005 cases) is ~5 days.
