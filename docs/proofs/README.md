...
| 20 | [20-adversarial-lemma-small-K.md](20-adversarial-lemma-small-K.md) | Theorem A: no `K <= 10` primes, two classes each at separation `3^{-1} (mod g)`, any phase, cover `(p_{K+1}^2-1)/6` consecutive columns.  Theorem B: `A(K) = 2, 5, 7, 16, 22, 28` at `K = 1..6`.  Lemmas: arc law, capacity, span lemma (a distance names the at most two primes that can span it), type lemma | no (finite certificates; `research/anchor235/r53/`) | The finite base of the open adversarial lemma `A(K) < (p_{K+1}^2-1)/6`, which is strictly stronger than the root; no induction step | Ziller-Morack `h_2` / A072753 (free classes); Stevens 1977 (one class); covering systems have no interval statement |
| 21 | [21-collision-laws.md](21-collision-laws.md) | The collision deficit of a gear pair: linear law `c(g,h;L+gh) = c(g,h;L)+4`; the shared-arc law (`a_g = a_h = a` forces `c(g,h;a+1) >= 1`), so every twin pair collides at `(g+4)/3` and the head collision is its `a = 2` case; `c(5,7;L) > 0` for all `L >= 21`; the arc floor (certificate); the block bound `L <= sum_B joint_max(B;L)`, the all-pairs form refuted, matching proves `K = 4` and block-4 proves `K = 5, 6`; the block-size ladder `1,2,2,3,4,5,6,7,8,8` | no (written proofs; finite certificates in `research/anchor235/r55/`) | Bounds adversarial covers and closes the counting-tight cases at `K = 4..6` with no solver; no induction step, and the order obstruction rules out any bounded-order interaction law | `separation_drives_K.md` N-S1 (same 4 as a mean); prior art not checked |
