...
## Round 20 (2026-08-18): the n-point correlation, and the adjacent-gap exclusion law

Tools: `research/npoint_autocorr.py`, `lagpair_predict.py`, `lambda_law.py`,
`padded_lag.py`, `bonferroni_gap.py`, `ie_pruning.py`.

### A. One closed form, three of my constructs

n points at offsets d_1..d_n are all exposed to gear q iff the phase avoids the
2n classes {t - d_i}. Two classes coincide iff d_i - d_j = 0 or +-2u. Hence

    c_q(d_1..d_n) = q - 2n + O,   O = #{ pairs with d_i - d_j = +-2u mod q },
    exact whenever q >= 2n.

Verified brute force, 16,500 checks over gears 5..43 and n = 1..5, **0
mismatches**. It subsumes: n=1 the exposed set (q-2); n=2 round 18's three-case
form; and "c_q > 0 forced when q > 2n" is round 17's completeness lemma. Three
constructs, one object, with the same three tooth-relationships throughout.

### B. THE ADJACENT-GAP EXCLUSION LAW - and it is a proof, not a statistic

For a lag pair the correlation ratio against independence is closed-form:

    Lambda(g1,g2) = prod_q [ c_q(0,g1,g1+g2)*(q-2) / (c_q(0,g1)*c_q(0,g2)) ].

Lambda = 0 exactly when some gear has c_q = 0 on the three-point shape. By the
completeness lemma a 3-point shape can only be blocked by gears q <= 2n = 6, so
**only gear 5 can ever do it** - gear 7 onward never blocks an adjacent gap
pair. Working out gear 5 (exposed residues {0,2,3}):

> **LAW.** Three consecutive openings with gaps (g1, g2) are IMPOSSIBLE
> whenever (g1 mod 5, g2 mod 5) lies in
> {(1,1), (1,3), (2,4), (3,1), (4,2), (4,4)} - 6 of 25 classes, 24%.
> **Forced, at every scale, in every machine containing gear 5.** A phase that
> fails mod 5 fails outright; CRT cannot rescue it. And the list is COMPLETE:
> no other gear can contribute an exclusion.
> **Scope: adjacent gaps only.** At separation j >= 2 the intervening openings
> are free, the offsets are not determined, and no exclusion follows.

The zeros are a CHECK on the proof, not the evidence for it. Cross-checked
against the Mechanic's independent census (`gap_pair_joint.csv`), six machines
y = 11..29:

    lag 1  : 1,589 populated cells across six machines, ZERO in a forbidden
             class - law holds everywhere.
    lag >=2: the SAME classes are heavily populated, up to 35,798,770 counts
             at y=29 - exactly as the scope predicts.

That two-sidedness is the point: the law forbids where it claims to and is
silent where it claims to be. (My own machine 19/23 scan found the same, 49 and
89 predicted-impossible cells with zero observed - now explained rather than
merely observed.)

### C. The padded lag, and an elementary law behind it

Since 2u_q = 2*6^{-1} = 3^{-1} (mod q), the enhancement condition
g = +-2u_q becomes 3g = +-1, i.e.

> **gear q is enhanced at lag g  <=>  q | 3g - 1  or  q | 3g + 1.**

Verified, 0 mismatches. So the arithmetic of a padded link is governed by the
**factorisation of 3q' +- 1**:

    q'   3q'-1  3q'+1   enhanced gears
    23      68     70   5, 7, 17
    29      86     88   11, 43
    31      92     94   23, 47
    37     110    112   5, 7, 11
...
Honest test against the measured padding supply (86, 6, 2090, 26367): sigma(q')
spans 3.3x while the supply share spans 330x, so the endpoint arithmetic
accounts for roughly a tenth of that erraticity. The rest is the interior - the
same verdict as round 18, now confirmed on the padding thread specifically.

### D. The interior disjunction: the exact object, and the cheap honest one
...
> **The correct object is an alternating sum of exposed-set correlations** -
> every single term in closed form from the construct. The disjunction does not
> factorise, but its EXPANSION is built entirely from factorising pieces.

The cheapest honest approximation is **Bonferroni truncation** - even depth is a
rigorous upper bound, odd depth a rigorous lower bound, no heuristic anywhere.
Measured against exact full-period counts at machine 19:

    g       exact    k=0       k=1       k=2      k=3     k=4    k=5    k=6
     8      10462   73710   -25886     16654    10270   10462  10462  10462
    12       2816   98280  -119652     60150    -7554    3308   2816   2816
    16        876   49140   -81998     53304   -13484    2488    816    876
    20        142  110565  -312222    356370  -203002   58905  -7430    402

Depth needed grows roughly like g/4 - Brun's problem in the machine's own
language, quantified. **But the construct prunes its own expansion**: c_5(S) = 0
whenever the point set occupies 4 or more residues mod 5, so most subsets
contribute exactly zero:

    g=20:  depth 2  36% pruned | depth 3  70% | depth 4  89% | depth 5  97%
    g=16:  depth 3  90% pruned | depth 4  98% | depth 5  99.6%

and the pruning strengthens exactly where the terms are most numerous. So the
honest statement is not "the disjunction blocks us" but: the exact object is
computable term by term, rigorously truncatable, and sparse - with the
sparsity read straight off the same small-gear structure as everything else.

### Proposed next chunk

The law of section B is a two-line kernel target (gear 5's exposed set is
{0,2,3}; three points at 0, g1, g1+g2; six residue pairs admit no phase) and
would be the first *forbidden-configuration* theorem in the Lean ledger. Beyond
that, the natural continuation of the same object: c_q at a lag pair is the
n=3 case, and part (D)'s flank sum is n=3 as well (left flank, word, right
flank) - so the same closed form applies to FS directly, with the word's span
as the middle offset. That is the first time my construct and the open
requirement have had the same arity, and it is where I would go next.
