...
def mutate_ast(node: ASTNode, mutation_rate: float = 0.1, prune_rate: float = 0.2, max_depth: int = 4) -> ASTNode:
    """Mutate an AST with improved mutation strategies including pruning."""
    ...
    if random.random() < mutation_rate:
        ...
        elif mutation_type < prune_rate + (1 - prune_rate) * 0.33:
            if node.op in ["var", "const", "named_const"]:
                choice = random.random()
                if choice < 0.33:
                    return ASTNode(op="const", value=random.randint(1, 10))
        ...
        elif mutation_type < prune_rate + (1 - prune_rate) * 0.66:
    ...
    if node.op in [
        ...
        if child_depth < max_depth:
            node.left = mutate_ast(node.left, mutation_rate, prune_rate, max_depth)
    elif node.op not in ["var", "const", "named_const"]:
        ...
        if right_depth < max_depth:
            node.right = mutate_ast(node.right, mutation_rate, prune_rate, max_depth)
...
def genetic_algorithm(
    ...
    for generation in range(generations):
        ...
        for _ in range(crossover_count):
            ...
            child = mutate_ast(child, mutation_rate=mutation_rate, prune_rate=prune_rate, max_depth=max_depth)
