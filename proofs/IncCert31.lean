/-
THE INCREMENT LAW AT 29->31, UPPER HALF, BY CASE-SPLIT LP DUALITY
(GENERATED root).
...
Here M = 29, q' = 31, s_min = 10 and F_2(29) = 55, so the width to
certify is W_inc = 65.  This is STRICTLY SMALLER than the (D) ladder's
budget width F(29) + 31, so it is a strictly harder obligation and is
NOT implied by the corresponding rung.

Every configuration of machine 31 has its held gears [5, 7] at exactly one
...
of 65 consecutive slots of machine 31 is fully blocked.

NO CENSUS HYPOTHESIS, NO PERIOD SCAN: the only inputs are the primes
up to 31 and 886 integers per case.
-/
import IncCert31C0
import IncCert31C1
import IncCert31C2
...
theorem nocase0 {p : ℕ} (e5 : p % 5 = 0) (e7 : p % 7 = 0)
    (hall : ∀ i, i < 65 → (gb5 (p % 5) i || gb7 (p % 7) i || gb11 (p % 11) i || gb13 (p % 13) i || gb17 (p % 17) i || gb19 (p % 19) i || gb23 (p % 23) i || gb29 (p % 29) i || gb31 (p % 31) i) = true) : False := by
...
/-- **`F(31) <= 65` by the case split at the INCREMENT width**:
every window of 65 consecutive slots contains an opening of
machine 31. -/
theorem no_run {p : ℕ} (hp : 1 ≤ p) :
    ∃ i < 65, Machine31.Exposed31 (p + i) := by
  by_contra hc
  push Not at hc
  have hall : ∀ i, i < 65 → (gb5 (p % 5) i || gb7 (p % 7) i || gb11 (p % 11) i || gb13 (p % 13) i || gb17 (p % 17) i || gb19 (p % 19) i || gb23 (p % 23) i || gb29 (p % 29) i || gb31 (p % 31) i) = true :=
...
theorem F_le (n : ℕ) : Machine31.g31 n ≤ 65 := by
  by_contra hcon
...
/-- **THE INCREMENT LAW'S UPPER HALF AT 29->31**: every gap of
machine 31 is at most `F_2(29) + s_min(31) = 55 + 10 = 65`.
No census hypothesis, no period scan - only the primes up to 31
and the 35 case certificates.  The matching LOWER half
(`F_2(29) >= 55`, a realisability statement no dual certificate
can carry) is `Increment.f2_29_ge`. -/
theorem inc_29_31 (n : ℕ) : Machine31.g31 n ≤ 55 + 10 :=
  F_le n
